...
export function AnalyzerBar({ value, label, type, maxHeight = 300, width = "120px" }: AnalyzerBarProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 md:gap-3">
      ...
      <div className="flex flex-col items-center text-xs md:text-sm">
        <span className="text-muted-foreground scale-90 md:scale-100">{label}</span>
        <span className={cn(
      ...
      {/* Bar container */}
      <div
        className={cn("bg-muted/30 rounded-t-lg overflow-hidden relative", width)}
        style={{ width: width === "120px" ? undefined : width, height: maxHeight }}
      >
        {/* Bar fill */}
        <div
          className={cn(
